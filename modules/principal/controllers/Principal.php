...
class Principal extends CI_Controller
{
	...
	public function raport_view()
	{
		...
		$data['grade'] = $grade;
		$data['student'] = $this->mprincipal->student_data($table, array('student.id_number' => $id_number, 'raport_periode.status' => 'active'))->row();
		...
		if($grade == 3){
			$data['raport'] = $this->mprincipal->raport_fmp_result($flag);
			$data['raport2'] = $this->mprincipal->dataRaportList($table, $flag)->result();
			$page_view = 'student_raport_y3';
		}

		$this->load->view("raport/{$page_view}", $data);
	}
	...
	public function save_fmp($id_raport_result, $id_raport_fmp, $result_form){
		...
		$pass = $this->mprincipal->last_result_fmp(array('raport_result'=>$id_raport_result,'flag_fmp'=>'pass'),FALSE)->row();
		$merit = $this->mprincipal->last_result_fmp(array('raport_result'=>$id_raport_result,'flag_fmp'=>'merit'),FALSE)->row();
		$distinction = $this->mprincipal->last_result_fmp(array('raport_result'=>$id_raport_result,'flag_fmp'=>'distinction'),FALSE)->row();
	...
	function revisi_score_result_by_principal(){
