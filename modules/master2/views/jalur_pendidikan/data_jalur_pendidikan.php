...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/master2/proses_jalur_pendidikan",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/master2/edit_jalur_pendidikan/'+id,
		function( response ) {
			$("#Modal").modal('show');
...
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/master2/delJalur_pendidikan/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
		...
		<div class="modal-content">
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      <label class="control-label col-sm-3 col-sm-3 col-xs-12">Nama Jalur</label>
                      <div class="col-md-9 col-sm-9 col-xs-12">
						<input type="hidden" class="form-control" id="id" name="id" value="kosong" />
                        <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama Jalur" />
                      </div>
                    </div>
			</form>
            </div>
