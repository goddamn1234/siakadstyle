...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/master2/proses_jenis_pendidikan",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/master2/edit_jenis_pendidikan/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_jenis']);
			$("#nama").val(response['nama_jenis']);
		}
	);
}
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/master2/delJenis_pendidikan/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
		...
		<div class="modal-content">
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      <label class="control-label col-sm-3 col-sm-3 col-xs-12">Nama Jenis</label>
