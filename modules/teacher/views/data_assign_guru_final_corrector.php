...
$(document).ready(function() {
	$("#isi").load('ajaxAssign_guru_final_corrector');
	$(".add").click(function(){
		$("#Modal").modal('show');
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/teacher/proses_assign_guru_final_corrector",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/teacher/edit_assign_guru_final_corrector/'+id,
		function( response ) {
			$("#Modal").modal('show');
...
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/teacher/delAssign_guru_final_corrector/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
