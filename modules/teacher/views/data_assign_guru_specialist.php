...
$(document).ready(function() {
	$("#isi").load('ajaxAssign_guru_specialist');
	$(".add").click(function(){
		$("#Modal").modal('show');
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/teacher/proses_assign_guru_specialist",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/teacher/edit_assign_guru_specialist/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_assign']);
			$("#guru").val(response['guru']);
			$("#specialist").val(response['specialist']);
		}
	);
}
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/teacher/delAssign_guru_specialist/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
		...
		<div class="modal-content">
			...
			<form class="form-horizontal form-label-left" id="fModal">
				<div class="form-group">
				 ...
				 <div class="form-group">
                  <label class="control-label col-sm-3 col-sm-3 col-xs-12">Specialist</label>
                  <div class="col-md-9 col-sm-9 col-xs-12">
				    <select name="specialist" class="form-control" id="specialist" placeholder="choose Status">
						    <?php foreach($specialist->result() as $sp){?>
								<option value="<?php echo $sp->id_special;?>"><?php echo $sp->nama_special;?></option>
							<?php }?>
					 </select>   
                  </div>
                </div>	
