...
<script>
    ...
    function ganti_syarat(){
        var syarat=$("#syarat_dioutput1").val();
        $("#syarat_dioutput").val(syarat);
    } 
...
<div class="col-md-12 col-sm-12 col-xs-12" style="min-height:460px;">
	<div class="x_panel">
      ...
      <?php  
        ...
        <div class="alert alert-info" style="background-color:#ff8181;border-color:#ff8181;">
          <strong>Syarat Kelulusan : </strong> <?php echo $pel->syarat;?>
          <input type="hidden" id="point" name="point" value="<?php echo $pel->point;?>" />
          <br>Muncul Di Output <select name="syarat_dioutput1" id="syarat_dioutput1" style="background:#dedede;color:#999" onchange="ganti_syarat()"><option value='T' <?php foreach($nilai as $nil) if ($nil->syarat_dioutput=='T') echo 'selected'?>>Tidak</option>
                             <option value='Y' <?php foreach($nilai as $nil) if ($nil->syarat_dioutput=='Y') echo 'selected'?>>Ya</option></select></td> 
        </div>
        <div class="alert alert-info" style="background-color:#ff8181;border-color:#ff8181;">
		...
		<div>
             <form class="form-horizontal form-label-left" id="fCatat" name="fCatat">
			    ...
			    <!--<input type="hidden" class="form-control" id="syarat_dioutput" name="syarat_dioutput" value="<?php echo $nil->syarat_dioutput;?>" />-->
			    
                <table class="table table-bordered">
                 <tr><td width="150px">Hasil<br>Muncul Di Output <select name="hasil_dioutput" id="hasil_dioutput"><option value='T' <?php foreach($nilai as $nil) if ($nil->hasil_dioutput=='T') echo 'selected'?>>Tidak</option>
                             <option value='Y' <?php foreach($nilai as $nil) if ($nil->hasil_dioutput=='Y') echo 'selected'?>>Ya</option></select></td>
                 ...
                 <tr>
                    <td width="150px">Hasil Konversi<br><input type="checkbox" id="manual" name="manual" value="Y" <?php foreach($nilai as $nil) if ($nil->manual=='Y') echo 'checked'; ?> onclick="cek_disable();"> Manual 
                    <br>Muncul Di Output <select name="nilai_dioutput" id="nilai_dioutput"><option value='T' <?php foreach($nilai as $nil) if ($nil->nilai_dioutput=='T') echo 'selected'?>>Tidak</option>
                             <option value='Y' <?php foreach($nilai as $nil) if ($nil->nilai_dioutput=='Y') echo 'selected'?>>Ya</option></select></td> 
