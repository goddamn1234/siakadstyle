...
$(document).ready(function() {
	$("#isi").load('ajaxLihat_nilai/<?php echo $murid;?>/<?php echo $final;?>/<?php echo $periode;?>');
...
function del(murid,pel){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/raport_mpp/hapus_nilai/"+murid+"/"+pel+"/<?php echo $periode?>",
					type:"post",
					success:function(){
					Lobibox.notify('success', {
					msg: 'Data Berhasil Dihapus'
					});
						$("#isi").load('ajaxLihat_nilai/<?php echo $murid;?>/<?php echo $final;?>');
					},
					error:function(respon){
					Lobibox.notify('error', {
					msg: 'Gagal Melakukan Hapus data'+respon
					});
					}
				})
			}
