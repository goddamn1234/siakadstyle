...
class mRaport_mpp extends CI_Model {
	...
	public function list_penilaian($periode,$guru){
		...
		return $this->db->query("select m.id_mapel,m.nama_mapel,m.kelas,kl.nama_kelas,j.nama_jenjang,m.guru,g.full_name,r.periode,r.tahun_akademik,count(m.id_mapel) as jumlah from student s
                                    ...
                                    left outer join mata_pelajaran m on(m.kelas=a.id_kelas) 
                                    ...
                                    union all
                                    select k.pelajaran,p.nama_mapel,sa.id_kelas,kl.nama_kelas,j.nama_jenjang,p.guru,g.full_name,r.periode,r.tahun_akademik,count(k.id_krs) from krs k 
                                    left outer join raport_periode r on(r.id_raport_periode=$periode) 
	...
	public function list_daftar_nilai($periode,$pelajaran,$kelas){
		...
		return $this->db->query("select s.id_number,s.full_name as nmurid,m.nama_mapel,kl.nama_kelas,j.nama_jenjang,n.nilai,n.hasil_konversi,r.id_raport_periode,r.periode,r.tahun_akademik,m.id_mapel from student s 
		                            left outer join raport_periode r on(r.id_raport_periode=$periode)
                                    left outer join student_assign a on(a.id_number=s.id_number and a.start_date<=r.awal and (a.end_date>=r.akhir or a.end_date is null)) 
                                    left outer join mata_pelajaran m on(m.kelas=a.id_kelas)
                                    left outer join kelas kl on(kl.id_kelas=a.id_kelas)
                                    ...
                                    union all
                                    select s.id_number,s.full_name,m.nama_mapel,kl.nama_kelas,j.nama_jenjang,n.nilai,n.hasil_konversi,r.id_raport_periode,r.periode,r.tahun_akademik,m.id_mapel from krs k 
	...
	public function list_kelas_koreksi($periode,$guru){
		...
		return $this->db->query("SELECT a.id_kelas,kl.nama_kelas,j.nama_jenjang,count(kl.nama_kelas) as jml FROM student m 
                ...
                left outer join kelas kl on(kl.id_kelas=a.id_kelas)
                left outer join jenjang_pendidikan j on(j.id_jenjang=kl.jenjang)
                where m.id_number in (select murid from nilai where sdh_submit='1' and periode=$periode) $wguru group by kl.id_kelas order by id_jenjang,kl.nama_kelas");
	}
	
	public function list_daftar_koreksi($periode,$guru,$kelas){
		if ($guru!='') $wguru=" and  m.id_number in (select murid from student_assign_fasilitator s left outer join guru g on(g.id_user=s.guru) where g.id_number='$guru')"; else  $wguru='';
      	if ($kelas!='') $wkelas="and a.id_kelas=$kelas"; else  $wkelas='';	
		return $this->db->query("SELECT m.id_number,m.full_name as nmurid,rp.periode,rp.id_raport_periode,rp.tahun_akademik,c.catatan,c.catatan_siswa,c.catatan_dioutput,c.catatan_siswa_dioutput,kl.nama_kelas FROM student m 
	...
	public function list_kelas_koreksi_final($periode,$filter){
	    if ($filter==true) { 
		   return $this->db->query("SELECT  a.id_kelas ,kl.nama_kelas,j.nama_jenjang,count(a.id_kelas) as jml FROM student m 
                ...
                left outer join jenjang_pendidikan j on(j.id_jenjang=kl.jenjang)
                left outer join catatan_final c on(c.periode=$periode and c.murid=m.id_number)
				where m.id_number in (select murid from nilai where sdh_dikoreksi='1' and periode=$periode)  group by kl.id_kelas order by j.id_jenjang, kl.nama_kelas");
	...
	public function list_raport_mpp($murid,$periode){
	    if ($periode<=21){
    		...
    		order by m.tipe_pelajaran,m.nama_mapel,CAST(k.ke as int)");
	       	...
	       	return $this->db->query("SELECT d.*,k.isi_kriteria,k.subjek,k.siklus,k.point,k.ke,m.nama_mapel,g.full_name as nguru,tp.nama_tipe,sk.nama_siklus,n.nilai as hasil,n.catatan,m.syarat,n.dioutput,n.evaluasi,n.hasil_konversi,n.hasil_dioutput,n.nilai_dioutput,n.syarat_dioutput,n.tercapai_dioutput,
	       	                        n.konversi,n.porsi_konversi,n.porsi_partisipasi,(select count(nilai) from nilai_detail ka where ka.pelajaran=m.id_mapel and ka.murid='$murid' and ka.periode=$periode) as jml FROM nilai_detail d 
    		...
    		order by m.tipe_pelajaran,m.nama_mapel,CAST(k.ke as SIGNED)");
	    }
	}
