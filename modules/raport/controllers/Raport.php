...
class Raport extends CI_Controller {
	...
	function proses_periode(){
		...
		$data = array(
			...
			'kepsek2'=> $this->input->post('kepsek2'),
			'kepsek3'=> $this->input->post('kepsek3'),
			'kepsek4'=> $this->input->post('kepsek4'),
		...
		$config['upload_path']          = './image/signature/kepsek';
		...
		$config['file_name']            = 'SMK_'.$this->input->post('id');
		$this->load->library('upload', $config);
		if($this->upload->do_upload('userfile2')){
			$data['image_ttd2'] = $this->upload->data('file_name');
		}
		$config['file_name']            = 'SMP_'.$this->input->post('id');
		$this->load->library('upload', $config);
		if($this->upload->do_upload('userfile3')){
			$data['image_ttd3'] = $this->upload->data('file_name');
		}
    	$config['file_name']            = 'SD_'.$this->input->post('id');
		$this->load->library('upload', $config);
		if($this->upload->do_upload('userfile4')){
			$data['image_ttd4'] = $this->upload->data('file_name');
		}
		...
		if($this->input->post('id') != 'kosong'){
			...
			$res['status'] = 'success';
			$res['message'] = 'Data berhasil dirubah';
		}else{
			$flag = array('status'=>'aktif');
	...
	function actPeriode($id){
		...
		if($cek->status == 'aktif'){
			...
			if($getData->num_rows() == 0){
				$this->allcrud->editdata('raport_periode',$flag,array('status'=>'aktif'));
			}else{
				$this->allcrud->editdata('errorrr',$flag,array('status'=>'aktif'));
			}
		}
	}
	...
	function point($id){
		...
		$periode = $this->allcrud->getdata('raport_periode',array('status'=>'aktif'),1,0)->row();
		$flag2 = array(
	...
	public function output2(){
		$this->auth();
		$data = otoritas('raport/output2',$this->session->userdata('sesRole'));
		$data['parent'] = 'Raport';
		$data['child'] 	= 'Output Raport MPP + Proyek';
		$data['page'] 	= 'output/data_output2';
		$data['periode']= $this->allcrud->listdata('raport_periode','id_raport_periode ASC',array());
		$data['jenis']= $this->allcrud->listdata('jenis_pendidikan','id_jenis',array());
		$data['jalur']= $this->allcrud->listdata('jalur_pendidikan','id_jalur',array());
		$data['jenjang']= $this->allcrud->listdata('jenjang_pendidikan','id_jenjang',array());
		$this->load->view('template',$data);
	}
	...
	public function filter_output2(){
		$this->auth();
		if ($this->session->userdata('sesRole')=='3'){
	         $guru=$this->session->userdata('sesId_number');
	         $guru_final = $this->mraport->cek_guru_final_corector($guru)->result();
	        foreach($guru_final as $gfinal);
	        if ($gfinal->hasil>0) $guru='';
	    } else $guru='';
	    $filter='';
	    $jenis = $this->input->post('jenis');
	    if ($jenis!='all')  $filter=" and jenis=$jenis";
	    $jalur = $this->input->post('jalur');
	    if ($jalur!='all')  $filter=$filter." and jalur=$jalur";
	    $jenjang = $this->input->post('jenjang');
	    if ($jenjang!='all')  $filter=$filter." and jenjang=$jenjang";
	    $grade = $this->input->post('grade');
	    if ($grade!='all') $filter=$filter." and tingkat=$grade";
	    $class = $this->input->post('class');
	    if ($class!='all') $filter=$filter." and k.id_kelas='$class'";
	    $periode = $this->input->post('periode');
	    if ($periode!='all') $filter=$filter." and n.periode='$periode'";
		$data = otoritas('raport/output',$this->session->userdata('sesRole'));
		$data['list'] = $this->mraport->list_raport_pub_gabungan($guru,$filter);
		$this->load->view('output/ajaxListRaport2',$data);
	}
	...
	function ajaxPublished(){
		...
		$q1 = $this->mraport->published(1)->result();
		$q2 = $this->mraport->published(2)->result();
		$q3 = $this->mraport->published(3)->result();
	...
	function published2(){
		$this->auth();
		$data = otoritas('raport/published2',$this->session->userdata('sesRole'));
		$data['parent'] = 'Raport';
		$data['child'] 	= 'Published';
		$data['page'] 	= 'published/data_published2';
		$this->load->view('template',$data);
	}
	
	function ajaxPublished2(){
		$this->auth();
		$data = otoritas('raport/published2',$this->session->userdata('sesRole'));
		if ($this->session->userdata('sesRole')=='3'){
	        $guru=$this->session->userdata('sesId_number');
		...
		$data['list1']= $this->mraport->list_raport_pub2($guru,'');
		$this->load->view('published/ajaxPublished2',$data);
	}
	
	function toggle_published2(){
		...
		if(!empty($id) && !empty($jenis)){
		    if (($jenis=="1")||($jenis=="3")){
			    $flag = array(
    				'murid' => $id, 'periode'=>$periode
			    );
			    if ($jenis=="1")
			         $update = array('pub'=>$tag);
			    else  $update = array('pub_konversi'=>$tag);
			    $tabel="raport_pub";
			} else {
			    $flag = array(
    				'murid' => $id, 'periode'=>$periode
			    ...
			    if ($jenis=="2")
			         $update = array('pub2'=>$tag);
			    else  $update = array('pub_konversi2'=>$tag);
			    $tabel="raport_pub";
			}
			
			if($this->allcrud->editdata($tabel,$flag,$update)){
				$res['status'] = 'success';
				$res['message'] = 'Data Berhasil Di update';
			}else{
				$res['status'] = 'failed';
					$res['message'] =$id.' + '.$jenis.' + '.$tag; 
				//$res['message'] = 'Data Gagal Di update';
			}
		}else{
			$res['status'] = 'failed';
	...
	public function publish_all2(){
		$publish = $this->input->post('publish');
			
			$update = array('pub'=> $publish,'pub_konversi'=> $publish, 'pub2'=> $publish, 'pub_konversi2'=> $publish);
			$this->allcrud->editdata('raport_pub',$flag,$update);
	...
	function create_raport(){ 
	    ...
	    $data['catat']=$this->mraport->list_catatan_mpp($murid,$periode);
     	...
     	$rpt=$this->mraport->list_proyek_di_raport_murid($murid,$periode)->result();
    	foreach($rpt as $raport){
    	    $header=$this->mraport->list_raport_proyek_header($raport->id_proyek);
    	    $data['header_pro']=$header;
    	    $jml_siklus = $this->mraport->jml_siklus_proyek($raport->id_proyek)->result();
    	    $data['jml_siklus']=0;
    	    foreach($jml_siklus as $jml_sik);
    	    $data['jml_siklus']=$jml_sik->jml;
    	    $jml_item = $this->mraport->max_item_siklus_proyek($raport->id_proyek)->result();
    	    $data['jml_item']=0;
    	    foreach($jml_item as $max_item);
    	    $data['max_item']=$max_item->jml;
    	    $data['siklus'] = $this->mraport->siklus_proyek($raport->id_proyek);
    	    $detail = $this->mraport->list_raport_proyek_detail($raport->id_proyek);
    	    $d=array();
    	    $n=array();
    	    $i=0;
    	    ...
    	    $data['catat']=$this->mraport->list_catatan_final($head->murid,$periode);
    	}
	    $html=$this->load->view('gabungan/design_raport.php',$data,true);
	    $filename	= 'Download';
        $this->load->library('pdfgenerator');
