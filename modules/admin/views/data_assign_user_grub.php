...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/admin/proses_assign_user_grub",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/admin/edit_assign_user_grub/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_assign']);
			$("#user").val(response['user']);
			$("#grub").val(response['grub']);
		}
	);
}
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/admin/delAssign_user_grub/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        ...
                        <input type="text" class="form-control" id="user" name="nama" placeholder="Nama Jenjang" />
                      </div>
                    </div>
                    <div class="form-group">
                      <label class="control-label col-sm-3 col-sm-3 col-xs-12">Nama User Grup</label>
                      <div class="col-md-9 col-sm-9 col-xs-12">
						<input type="hidden" class="form-control" id="id" name="id" value="kosong" />
                        <input type="text" class="form-control" id="grup" name="nama" placeholder="Nama Jenjang" />
                      </div>
                    </div>
			</form>
            </div>
