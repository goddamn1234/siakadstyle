...
$(document).ready(function() {
	$("#isi").load('ajaxAssign_user_divisi');
	$(".add").click(function(){
		$("#Modal").modal('show');
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/admin/proses_assign_user_divisi",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/admin/edit_Assign_user_divisi/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_assign']);
			$("#user").val(response['full_name']);
...
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/admin/delAssign_user_divisi/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        <select name="staf" class="form-control" id="staf" placeholder="choose Status">
						    <?php foreach($divisi_list->result() as $div){?>
