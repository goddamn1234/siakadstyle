...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/admin/proses_assign_user_staf",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/admin/edit_assign_user_staf/'+id,
		function( response ) {
			$("#Modal").modal('show');
...
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/admin/delAssign_user_staf/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
					msg: 'Data Berhasil Dihapus'
					});
						$("#isi").load('ajaxAssign_user_grub');
					},
					error:function(){
					Lobibox.notify('error', {
		...
		<div class="modal-content">
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      <label class="control-label col-sm-3 col-sm-3 col-xs-12">Nama User</label>
                      <div class="col-md-9 col-sm-9 col-xs-12">
						<input type="hidden" class="form-control" id="id" name="id" value="kosong" />
                        <select name="user" class="form-control" id="user" placeholder="choose Status">
						    <?php foreach($user_list->result() as $usr){?>
								<option value="<?php echo $usr->id_user;?>"><?php echo $usr->full_name;?></option>
							<?php }?>
						</select>   
                      </div>
                    </div>
                    <div class="form-group">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        <select name="staf" class="form-control" id="staf" placeholder="choose Status">
						    <?php foreach($staf_list->result() as $staf){?>
								<option value="<?php echo $staf->id_staf;?>"><?php echo $staf->nama_staf;?></option>
							<?php }?>
						</select>   
                      </div>
                    </div>
