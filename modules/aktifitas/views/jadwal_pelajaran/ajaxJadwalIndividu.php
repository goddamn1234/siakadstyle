   <table id="datatable9" class="table table-striped table-bordered">
		...
		<tbody>
	        <?php $no=1;
	        foreach($list_individu->result() as $row){?>
				  <tr>
					...
					<td><?php if($row->jadwal!=''){?>
					        <a href="<?php echo base_url()?>/uploads/jadwal_individu/<?php echo $row->jadwal;?>" target='_blank' class="btn btn-primary"><i class="fa fa-eye"></i> Lihat File</a>
					     <?php } else {?> 
					         <span style="color:red;font-size:10px">Belum Ada Upload</span>
					     <?php } ?>
					</td>
					<td>
						<button <?php echo $otoU;?> class="btn btn-info btn-xs" type="button" onclick="upload_individu(<?php echo $row->id_raport_periode;?>,<?php echo $row->id_number;?>)" data-toggle="tooltip" data-placement="top" title="Upload"><i class="fa fa-upload"></i></button>
						<button <?php echo $otoD;?> class="btn btn-danger btn-xs" type="button" onclick="del_individu(<?php echo $row->id_raport_periode;?>,<?php echo $row->id_number;?>)" data-toggle="tooltip" data-placement="top" title="hapus"><i class="fa fa-trash"></i></button>								
					</td>
				  </tr>
			  <?php $no++; };  ?>
		</tbody>
