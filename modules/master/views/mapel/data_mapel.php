...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/master/proses_mapel",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/master/editMapel/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_mapel']);
			$("#nama").val(response['nama_mapel']);
			$("#tingkat").val(response['tingkat_kelas']);
			$("#req").val(response['achiev_point']);
...
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/master/delMapel/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
	...
	<div class="modal-dialog modal-lg" style="width:500px;">
		<div class="modal-content">
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      <label class="control-label col-sm-3 col-sm-3 col-xs-12">Point Req.</label>
                      <div class="col-md-9 col-sm-9 col-xs-12">
						<input type="number" class="form-control" name="req" id="req"/>
                      </div>
                    </div>
					<div class="form-group">
