...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/master/proses_divisi",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/master/edit_divisi/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_divisi']);
			$("#nama").val(response['nama_divisi']);
		}
	);
}
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/master/delDivisi/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        ...
                        <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama Divisi" />
                      </div>
                    </div>
			</form>
            </div>
