...
$(document).ready(function() {
	...
	$("#save").click(function(){
		...
		if(nama != ""){
			$.ajax({
			url :"<?php echo site_url();?>/master/proses_category",
				type:"post",
...
function edit(id){
	$.getJSON('<?php echo site_url() ?>/master/editCategory/'+id,
		function( response ) {
			$("#Modal").modal('show');
			$(".modal-title").html('Edit Data');
			$("#id").val(response['id_category']);
			$("#nama").val(response['nama_category']);
		}
	);
}
function del(id){
	 Lobibox.confirm({
		 ...
		 callback: function ($this, type) {
			if (type === 'yes'){
				$.ajax({
					url :"<?php echo site_url()?>/master/delCategory/"+id,
					type:"post",
					success:function(){
					Lobibox.notify('success', {
		...
		<div class="modal-content">
			...
			<form class="form-horizontal form-label-left" id="fModal">
					...
					<div class="form-group">
                      <label class="control-label col-sm-3 col-sm-3 col-xs-12"> Name</label>
