...
class Raport_bipp extends CI_Controller {

	public function __construct() {
		parent::__construct();
        $this->load->model ('mRaport_bipp', '', TRUE);
	...
	function penilaian(){
        $this->auth();
		$data = otoritas('raport_bipp/penilaian',$this->session->userdata('sesRole'));
		$data['parent'] = 'Raport BIPP';
	...
	function ajaxPenilaian(){
	    ...
	    if ($id_periode==''){
	      $periode = $this->mRaport_bipp->periode_aktif()->result();
	      foreach($periode as $period);
	      $id_periode=$period->id_raport_periode;
	    }  
	    $data['periode'] = $id_periode;
	    $data['list'] = $this->mRaport_bipp->list_penilaian($id_periode,$guru);
	...
	function ajaxDaftar_nilai(){
	    ...
	    $data['list']	= $this->mRaport_bipp->list_daftar_nilai($per,$pelajaran,$kelas);
	...
	function pengisian_nilai(){
		...
		$data['kelas']=$this->mRaport_bipp->lihat_kelas($id_number)->result();
		...
		$data['fasilitator'] = $this->mRaport_bipp->lihat_fasilitator($id_number)->result();
		$data['parent'] = 'Raport BIPP';
		$data['child'] 	= 'Pengisian Nilai';
	...
	function ajaxPengisian_nilai(){
		...
		$data['list'] = $this->mRaport_bipp->list_kriteria_nilai($id_periode,$id_number,$id_mapel);
	...
	function koreksi_nilai(){
        $this->auth();
		$data = otoritas('raport_bipp/koreksi_nilai',$this->session->userdata('sesRole'));
		$data['parent'] = 'Raport BIPP';
    ...
    function ajaxKoreksi_nilai(){
	    ...
	    $data['periode']=$id_periode;
	    $data['list']	= $this->mRaport_bipp->list_kelas_koreksi($id_periode,$guru);
	...
	function ajaxDaftar_koreksi_nilai(){
	    ...
	    $data['list']	= $this->mRaport_bipp->list_daftar_koreksi($id_periode,$guru,$kelas);
	...
	function ajaxLihat_nilai(){
	    ...
	    $data['list']	= $this->mRaport_bipp->list_lihat_nilai($id_periode,$murid);
	    $data['periode']= $id_periode;
	...
	function proses_simpan_nilai(){
		$this->auth();
		$pel = $this->mRaport_bipp->lihat_pelajaran($this->input->post('id'))->result();
	    foreach($pel as $mpel);
