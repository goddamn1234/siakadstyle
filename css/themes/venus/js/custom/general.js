...
jQuery(document).ready(function(){
	...
	jQuery('.gridmenu').click(function(){
		if(!jQuery(this).hasClass('active')) {
			jQuery('.gridmenudrop').show();
			jQuery(this).addClass('active');
		} else {
			jQuery('.gridmenudrop').hide();
			jQuery(this).removeClass('active');
		}
		//remove notification box if visible
		jQuery('.notification').removeClass('active');
	...
	// submenuDrop
	jQuery(document).click(function(event) {
	var ud = jQuery('.gridmenudrop');
	var nb = jQuery('.noticontent');

	//hide user drop menu when clicked outside of this element
	if(!jQuery(event.target).is('.gridmenudrop')
		&& !jQuery(event.target).is('.userdata')
		&& ud.is(':visible')) {
			ud.hide();
			jQuery('.gridmenu').removeClass('active');
	}
